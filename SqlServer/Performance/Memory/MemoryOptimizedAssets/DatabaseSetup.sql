...
Execute 
	...
	-- This is for SQL on a linux container or operating system. Alter the slash to \ if you want Windows
	SELECT TOP 1 
		physical_name,
...
CREATE DATABASE [TestDB]
 CONTAINMENT = NONE
 ON  PRIMARY 
( NAME = N'TestDB', FILENAME = N'<path, NVARCHAR(520), >/TestDB.mdf' , SIZE = 8192KB , FILEGROWTH = 65536KB )
 LOG ON 
( NAME = N'TestDB_log', FILENAME = N'<path, NVARCHAR(520), >/TestDB_log.ldf' , SIZE = 8192KB , FILEGROWTH = 65536KB )
 COLLATE Latin1_General_100_BIN2
GO
ALTER DATABASE [TestDB] ADD FILEGROUP [MemOptimied] CONTAINS MEMORY_OPTIMIZED_DATA 
GO
ALTER DATABASE [TestDB] ADD FILEGROUP [MemOptimised]
GO
ALTER DATABASE TestDB ADD FILE(name='memOptimized1', filename='<path, NVARCHAR(520), >/memOptimized') TO FILEGROUP [MemOptimied]
GO
...
END
GO
CREATE PROCEDURE dbo.uspTestMemoryOptimizedTables @IsOptimized BIT, @LoopCount INT, @ShowReport BIT
AS
...
BEGIN
	...
	(
		...
		CONSTRAINT pk_ID PRIMARY KEY NONCLUSTERED(ID DESC)
...
VALUES('Inserts: Iteratively', @StartTime, @EndTime, @LoopCount, @BatchExecutionTime, @BatchID, @IsOptimized)
